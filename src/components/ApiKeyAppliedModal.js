    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md p-8 text-center relative transform transition-all" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={onClose} className="mt-6 w-full bg-custom-teal hover:bg-custom-teal-dark text-white font-bold py-2.5 px-4 rounded-lg shadow-md hover:shadow-lg transition-all">{uiText.gotIt}</button>
            </div>
        </div>
    );
};
