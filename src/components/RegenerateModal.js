...
const RegenerateModal = ({ isOpen, onClose, content, onRegenerate, contentType }) => {
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-2xl p-6 relative flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 border-b pb-4 dark:border-slate-700">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">Edit & Regenerate</h3>
                ...
                <div className="flex-grow overflow-y-auto pr-2">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h4 className="font-semibold mb-2 text-gray-700 dark:text-gray-300">Original Version</h4>
                        ...
                        <div>
                            <h4 className="font-semibold mb-2 text-gray-700 dark:text-gray-300">New Version</h4>
                            <div className="bg-gray-100 dark:bg-slate-900/50 p-4 rounded-lg text-sm min-h-[150px] flex items-center justify-center">
                                {isRegenerating ? (
                                    <div className="text-center">
                                        ...
                                        <p className="text-xs mt-2">AI is working...</p>
                                    </div>
                                ) : newVersion ? (
                                    ...
                                    <p className="text-gray-400">New version will appear here.</p>
                                )}
                            </div>
                        </div>
                    </div>
                    <div className="mt-4">
                        <label htmlFor="instructions" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Revision Instructions (Free Form)</label>
                        <textarea id="instructions" value={instructions} onChange={e => setInstructions(e.target.value)} rows="3" className="w-full p-2 rounded-lg bg-gray-100 dark:bg-slate-700 focus:ring-2 ring-custom-teal" placeholder="e.g., Make it funnier, target teenagers, use more formal language..."></textarea>
                    </div>
                </div>
